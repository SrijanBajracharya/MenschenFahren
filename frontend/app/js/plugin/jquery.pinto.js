...
(function ( $ ) {
    ...
    Pinto.prototype = {
         ...
         *        column number
         * @param {number} -
         *        item position inside the column
         */
        ...
        layout : function () {
            ...
            var self = this, items = this.el.find(this.itemSelector), width = this.el.innerWidth(), itemWidth = this.itemWidth, gapX = parseInt(this.gapX || 0), gapY = parseInt(this.gapY || 0), offset = 0, colsCount = 0;
            ...
            if (this.fitWidth) {
                ...
                // calculate the offset based on the alignment of columns to the
                // parent container
            ...
            items.each(function ( index, item ) {
                var $item = $(item), i = self.getSmallestIndex(colsH);
                ...
                $item.css({
                    ...
                    width : itemWidth - ($item.outerWidth() - $item.width()) // control
                                                                                // padding
                                                                                // and
                                                                                // border
                });
                ...
                if (typeof self.onItemLayout == "function") { // make sure the
                                                                // callback is a
                                                                // function
                    self.onItemLayout.call(self, $item, i, cols[i]); // brings
                                                                        // the
                                                                        // scope
                                                                        // to
                                                                        // the
    ...
    /**
     * @param CfgOrCmd -
     *        config object or command name you may set any public property (see
     *        above); you may use .pinto("layout") to call the layout function
     * @param CmdArgs -
     *        some commands may require an argument
     */
    $.fn.pinto = function ( CfgOrCmd, CmdArgs ) {
        var dataName = "pinto", instance = this.data(dataName);
        ...
        return this.each(function () {
            var el = $(this), instance = el.data(dataName), config = $.isPlainObject(CfgOrCmd) ? CfgOrCmd : {};
